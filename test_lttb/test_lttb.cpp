...
* 这个算法的逻辑是这样的。
* 输入两个参数。第一个参数是一个点列表。第二个参数是一个组个数。
* 首先
* 计算逻辑如下：
* 1. 把起始点加入拟合结果。因为我们准备拟合的点应该是以起始点开始。
*    这里定义为点A。
* 2. 之后我们需要根据输入的组个数，计算点列表中每一个组的大小。
*    计算公式为：
*        组大小 = (点个数 - 2) / (组个数 - 2)
*    点个数减2和组个数减2的原因是除去首尾。因为起始点和结束点自成一组。
* 之后我们一个组一个组计算，计算每一个组里面的拟合值。
* 3. 我们首先计算这个组里面的平均点。
*    方法就是把组里面所有的X求和除以点个数。所有的Y求和除以点个数。
*    这里定义为点C。
* 4. 遍历这个组里面的每一个点。这里定义为点B。
*    我们逐个计算每一个点B和点A，还有点C组成的三角形△ABC的面积。
*    
* 5. 在找到面积最大的那个三角形的时候，
*    这个三角形对应的点B就是我们选中的拟合点。
*    同时我们需要记下这个选中点的索引。
*    因为这个选中点在下一组中会作为点A，进行三角形的面积运算。
* 
*    这个就是这个算法中最巧妙的部分。
