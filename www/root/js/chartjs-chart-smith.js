...
class SmithScale extends Chart.Scale {
	...
	fit() {
		...
		const fontSize = helpers$1.getValueOrDefault(me.options.ticks.fontSize, Chart.defaults.global.defaultFontSize);

		if (me.options.ticks.display) {
			const fontStyle = helpers$1.getValueOrDefault(me.options.ticks.fontStyle, Chart.defaults.global.defaultFontStyle);
			const fontFamily = helpers$1.getValueOrDefault(me.options.ticks.fontFamily, Chart.defaults.global.defaultFontFamily);
			const labelFont = helpers$1.fontString(fontSize, fontStyle, fontFamily);
			...
			helpers$1.each(me.xTicks, (xTick, index) => {
				if (xTick !== 0) {
	...
	draw() {
		...
		if (me.options.display) {
			...
			if (me.options.ticks.display) {
				...
				me.ctx.fillStyle = helpers$1.getValueOrDefault(me.options.ticks.fontColor, Chart.defaults.global.defaultFontColor);

				helpers$1.each(me.rLabels, (rLabel, index) => {
				...
				helpers$1.each(me.xLabels, (xLabel, index) => {
